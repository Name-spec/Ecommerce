...
def place_order():
    try:
        if request.method == 'POST':
            ...
            user = db.query(User).filter(User.username == session['username']).first()
            obj = Order(first_name, last_name, main_address, temp_address, phone, email, zip, tp, discount, cart_total,
                        order_total, user)
            db.add(obj)
            db.commit()
            order = db.query(Order).join(User).filter(User.username == session['username']).order_by(
                Order.id.desc()).first()
            for x in cart:
        ...
        return render_template("error1.html", Message="Something went wrong\n")
    ...
    try:
        if 'username' in session:
            if (session['username'] == adminUsername):
                ...
                prod = db.query(Product).filter(Product.id == id).first()
                user = db.query(User).filter(User.username == session['username']).first()
                check = db.query(Wishlist).filter(
                    (Wishlist.userId == user.id) & (Wishlist.productId == prod.id)).first()
...
def categories_page2():
    try:
        ...
        if 'username' in session:
            if session['username'] == adminUsername:
                return render_template("categories-page2.html", login=True, admin=True, data=data,
                                       categories=sub_categories, main=name)
            else:
                cart, wish, cart_total = return_wishlist_and_cart()
                return render_template("categories-page2.html", login=True, admin=False, data=data,
                                       categories=sub_categories, main=name, wish=wish, cart=cart,
                                       cart_total=cart_total)
        else:
            return render_template("categories-page2.html", login=False, admin=False, data=data,
                                   categories=sub_categories, main=name)
    except:
        return render_template("error1.html", Message="Something went wrong\n")
...
def list_product():
    try:
        ...
        if 'username' in session:
            if (session['username'] == adminUsername):
                return render_template("list-product.html", login=True, admin=True, data=data, items=products,
                                       name=name)
            else:
                cart, wish, cart_total = return_wishlist_and_cart()
                return render_template("list-product.html", login=True, admin=False, data=data, items=products,
                                       name=name, wish=wish, cart=cart,
                                       cart_total=cart_total)
        else:
...
def add_category():
    if request.method == 'POST':
        ...
        current_directory = "static/assets/img/" + m + "/"
...
def view_product():
    try:
        ...
        return render_template("view-product.html", login=True, admin=True, data=data, sub_cat_name=value,
                               items=sub_category)
    except:
        return render_template("error1.html", Message="Something went wrong\n")
...
def home():
    try:
        ...
        if 'username' in session:
            if (session['username'] == adminUsername):
                return render_template("index.html", log=True, admin=True, data=data, new=new_arrive, wish=wish,
                                       cart=cart, cart_total=cart_total)
            else:
                cart, wish, cart_total = return_wishlist_and_cart()
                return render_template("index.html", log=True, admin=False, data=data, new=new_arrive, wish=wish,
                                       cart=cart, cart_total=cart_total)
        return render_template("index.html", log=False, admin=False, data=data, new=new_arrive, wish=wish, cart=cart,
                               cart_total=cart_total)
    except:
        return render_template("error1.html", Message="Something went wrong\n")
